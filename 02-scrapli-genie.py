...
parser = argparse.ArgumentParser(description="Validate startup and running config differences on Cisco devices.")
parser.add_argument('--host', action='store',help='Specify the hostname to be queried i.e. lab-host-01.lab.local', type=str)
# Add debug argument, set to False by default unless required
parser.add_argument('--debug', action='store_true',help='Provide additional debugging output. True for debugging, set to False by default')
...
try:
  ...
  if args.debug is True:
    print(Fore.MAGENTA + f"Attempting API call - {url}{api_path}{hostname}{command_api_path}{command}")
